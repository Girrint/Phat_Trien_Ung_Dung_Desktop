...
namespace KetQuaXoSo
{
	internal class XoSoHelper
	{
				...
				using (var httpClient = new HttpClient())
				{
					...
					using (var reader = XmlReader.Create(stream, settings))
					{
						...
						foreach (var item in feed.Items)
						{
							...
							var kq = new KetQuaXoSo { Ngay = item.Title.Text };
							...
							if (matchDate.Success)
							{
								...
								// Nếu chỉ có dd/MM hoặc dd-MM thì thêm năm hiện tại
							...
							for (int i = 0; i < tokens.Count; i++)
							{
								...
								// Trường hợp đặc biệt: giá trị giải 7 và 8 dính nhau dạng "xxx:yy"
								...
								// TH "7:" và token sau có dạng "xxxx:" => gộp 7,8
								...
								{
									var parts = token.Split(new char[] { ':' }, 2, StringSplitOptions.RemoveEmptyEntries);
									currentKey = parts[0];
		...
		public class Program
		{
			...
			public static string CreateRSSLink(string prv)
			{
			...
			public static string KiemTraVeSo(string ve, KetQuaXoSo kq)
			{
				...
				{
					if (string.IsNullOrEmpty(giai)) return false;
					return veso.EndsWith(giai);
				};

				// chỉ xét nếu đài thực sự có giải 8
				if (kq.CoGiai8 && Match(ve, kq.Giai8))
					return "Trúng giải 8";
				...
				if (!string.IsNullOrEmpty(kq.DacBiet) && ve.EndsWith(kq.DacBiet.Substring(kq.DacBiet.Length - 2)))
